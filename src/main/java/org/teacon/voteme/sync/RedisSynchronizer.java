...
public final class RedisSynchronizer implements VoteSynchronizer {
    private static final String ARTIFACT = "voteme:artifact"; // voteme:artifact:<artifact-id>
    private static final String COMMENTS = "voteme:comments"; // voteme:artifact:<artifact-id>:<voter-id>
    private static final String VOTE = "voteme:vote"; // voteme:vote:<artifact-id>:<category-id>:<voter-id>
    private static final String VOTE_DISABLED = "voteme:vote_disabled"; // voteme:vote_disabled:<artifact-id>:<category-id>
    private static final String VOTE_STATS = "voteme:vote_stats"; // voteme:vote_stats:<artifact-id>:<category-id>:<role-id>
                    ...
                    .thenRunAsync(() -> {
                        if (succeed.getAcquire()) {
                            ...
                            for (Vote affectedVote : affectedVotes) {
                                toChannelMessage(affectedVote.key(), this::publish);
                            }
                            for (VoteStatsKey key : affectedStatsMap.keySet()) {
                                toChannelMessage(key, this::publish);
                            }
                        } else {
    ...
    private static String toRedisKey(AnnounceKey<?> announceKey) {
        return toChannelMessage(announceKey, (channel, message) -> channel + ":" + message);
    }

    private static <T> T toChannelMessage(AnnounceKey<?> announceKey, BiFunction<String, String, T> function) {
        if (announceKey instanceof ArtifactKey key) {
            return function.apply(ARTIFACT, key.artifactID().toString());
        }
        if (announceKey instanceof CommentsKey key) {
            return function.apply(COMMENTS, key.artifactID() + ":" + key.voterID());
        }
        if (announceKey instanceof VoteKey key) {
            return function.apply(VOTE, key.artifactID() + ":" + key.categoryID() + ":" + key.voterID());
        }
        if (announceKey instanceof VoteDisabledKey key) {
            return function.apply(VOTE_DISABLED, key.artifactID() + ":" + key.categoryID());
        }
        if (announceKey instanceof VoteStatsKey key) {
            return function.apply(VOTE_STATS, key.artifactID() + ":" + key.categoryID() + ":" + key.roleID());
        }
    ...
    private static AnnounceKey<?> fromRedisKey(String redisKey) {
        ...
        return fromChannelMessage(redisKeyParts[0] + ":" + redisKeyParts[1], redisKeyParts[2]);
    }

    private static AnnounceKey<?> fromChannelMessage(String channel, String message) {
        switch (channel) {
            case ARTIFACT -> {
                UUID artifactID = UUID.fromString(message);
    ...
    private RedisFuture<Long> publish(String channel, String message) {
        VoteMe.LOGGER.info("Publishing message {} to redis channel {}", message, channel);
        ...
        if (announcement instanceof Artifact artifact) {
            ...
            if (artifact.name().isEmpty()) {
                async.del(toRedisKey(artifact.key()))
                        .thenComposeAsync(res -> toChannelMessage(artifact.key(), this::publish), this.server);
            } else {
                ImmutableMap.Builder<String, String> builder = ImmutableMap.builder();
        ...
        if (announcement instanceof Comments comments) {
            ...
            if (comments.comments().isEmpty()) {
                async.del(toRedisKey(comments.key()))
                        .thenComposeAsync(res -> toChannelMessage(comments.key(), this::publish), this.server);
            } else {
        ...
        if (announcement instanceof VoteDisabled voteDisabled) {
            ...
            if (voteDisabled.disabled().isEmpty()) {
                async.del(toRedisKey(voteDisabled.key()))
                        .thenComposeAsync(res -> toChannelMessage(voteDisabled.key(), this::publish), this.server);
            } else {
    ...
    private class PubSubListener extends RedisPubSubAdapter<String, String> {
        ...
        public void message(String channel, String message) {
            AnnounceKey<?> announceKey = fromChannelMessage(channel, message);
            VoteMe.LOGGER.info("Retrieving message {} from redis channel {}", message, channel);
            RedisSynchronizer.this.dispatch(announceKey).thenAccept(RedisSynchronizer.this.queued::offer);
        }
    }
}
